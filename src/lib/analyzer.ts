 ...
 * then suggests issues for Claude Code to fix.
 */
...
export function analyzeCommits(
      ...
      if (p.re.test(msg)) {
        ...
        if (!seen.has(key)) {
          ...
          findings.push({
            id: `F-${commit.sha}-${findings.length}`,
            title: p.title,
            description: `Commit \`${commit.sha}\`: "${msg}"\n\nThis looks like a code-quality concern. Claude Code should clean it up.`,
            severity: p.severity,
  ...
  // Always add a baseline architecture reminder if few findings
...
1. Investigate the related code
2. Fix the issue properly
3. Add tests if needed
4. Close this issue when done

---
*CodeBoss (free automated monitor)*`;
}
